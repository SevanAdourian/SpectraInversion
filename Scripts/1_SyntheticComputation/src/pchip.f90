...
!*****************************************************************************80
!
!! DD1MACH returns real ( kind = 8 ) real machine constants.
!
!  Discussion:
!
...
!    exponent, then
!
!      DD1MACH(1) = B^(EMIN-1), the smallest positive magnitude.
!      DD1MACH(2) = B^EMAX*(1-B^(-T)), the largest magnitude.
!      DD1MACH(3) = B^(-T), the smallest relative spacing.
!      DD1MACH(4) = B^(1-T), the largest relative spacing.
!      DD1MACH(5) = log10(B).
!
!  Licensing:
!
...
!    1 <= I <= 5.
!
!    Output, real ( kind = 8 ) DD1MACH, the value of the chosen parameter.
!
  implicit none
  ...
  if ( i < 1 ) then
    ...
    dd1mach = 4.450147717014403D-308
  else if ( i == 2 ) then
    dd1mach = 8.988465674311579D+307
  else if ( i == 3 ) then
    dd1mach = 1.110223024625157D-016
  else if ( i == 4 ) then
    dd1mach = 2.220446049250313D-016
  else if ( i == 5 ) then
    dd1mach = 0.301029995663981D+000
  else if ( 5 < i ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'DD1MACH - Fatal error!'
    write ( *, '(a)' ) '  The input argument I is out of bounds.'
...
!
  ...
  real ( kind = 8 )  D1, D2, DELTA, DD1MACH
  integer ( kind = 4 ) ISMON, ITRUE
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  real ( kind = 8 ) DBVALU, DD1MACH
  EXTERNAL  DBVALU, DPCHBS, DD1MACH
...
!*****************************************************************************80
!
!! II1MACH returns integer machine constants.
!
!  Discussion:
!
!    Input/output unit numbers.
!
!      II1MACH(1) = the standard input unit.
!      II1MACH(2) = the standard output unit.
!      II1MACH(3) = the standard punch unit.
!      II1MACH(4) = the standard error message unit.
!
!    Words.
!
!      II1MACH(5) = the number of bits per integer storage unit.
!      II1MACH(6) = the number of characters per integer storage unit.
!
!    Integers.
!
...
!    where 0 <= X(1:S-1) < A.
!
!      II1MACH(7) = A, the base.
!      II1MACH(8) = S, the number of base A digits.
!      II1MACH(9) = A^S-1, the largest integer.
!
...
!
!      II1MACH(10) = B, the base.
!
!    Single precision
!
!      II1MACH(11) = T, the number of base B digits.
!      II1MACH(12) = EMIN, the smallest exponent E.
!      II1MACH(13) = EMAX, the largest exponent E.
!
!    real ( kind = 8 )
!
!      II1MACH(14) = T, the number of base B digits.
!      II1MACH(15) = EMIN, the smallest exponent E.
!      II1MACH(16) = EMAX, the largest exponent E.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
...
!    1 <= I <= 16.
!
!    Output, integer ( kind = 4 ) II1MACH, the value of the chosen parameter.
!
  implicit none

  integer ( kind = 4 ) i
  integer ( kind = 4 ) ii1mach

  if ( i < 1 ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'II1MACH - Fatal error!'
    write ( *, '(a)' ) '  The input argument I is out of bounds.'
...
!
  ...
  integer ( kind = 4 ) LUN, KPRINT, IPASS
  real ( kind = 4 ) BVALU, RR1MACH
  EXTERNAL  BVALU, PCHBS, RR1MACH
  integer ( kind = 4 ) I, IERR, IFAIL, INBV, J, KNOTYP, K, N, NDIM, NKNOTS
  PARAMETER  (N = 9)
...
!*****************************************************************************80
!
!! RR1MACH returns single precision real machine constants.
!
!  Discussion:
!
...
!    RR1MACH will return the following values:
!
!      RR1MACH(1) = B^(EMIN-1), the smallest positive magnitude.
!      RR1MACH(2) = B^EMAX*(1-B^(-T)), the largest magnitude.
!      RR1MACH(3) = B^(-T), the smallest relative spacing.
!      RR1MACH(4) = B^(1-T), the largest relative spacing.
!      RR1MACH(5) = log10(B)
!
!  Licensing:
!
...
!    1 <= I <= 5.
!
!    Output, real ( kind = 4 ) RR1MACH, the value of the chosen parameter.
!
  implicit none
  ...
  if ( i < 1 ) then
    ...
    rr1mach = 1.1754944E-38
  else if ( i == 2 ) then
    rr1mach = 3.4028235E+38
  else if ( i == 3 ) then
    rr1mach = 5.9604645E-08
  else if ( i == 4 ) then
    rr1mach = 1.1920929E-07
  else if ( i == 5 ) then
    rr1mach = 0.3010300E+00
  else if ( 5 < i ) then
    write ( *, '(a)' ) ' '
    write ( *, '(a)' ) 'RR1MACH - Fatal error!'
    write ( *, '(a)' ) '  The input argument I is out of bounds.'
...
!  ERROR MESSAGE UNIT INSTEAD.  II1MACH(4) RETRIEVES THE STANDARD
!  ERROR MESSAGE UNIT.
!
  N = II1MACH(4)
  DO I=1,NUNIT
...
!                default unit, as defined by the II1MACH machine
