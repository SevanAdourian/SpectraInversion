...
c**************************************************************************
c
c  HL updated features:
c  now forced at a set frequency for direct solution method.
c  this affects the subroutine detqn or search for the string "HL1"
c  to find the position.  This corrects for dispersion
c  I have also updated the imaginary part to account for frequency dependent
c  attenuation.  Search for the string "HL2"
c
c  So to fully account for anelasticity consistent with the coupling code
c  (1) search "HL1" and adjust fct in subroutine detqn 
c  (2) search "HL2" and adjust qmod to map qmod from wref to wf
c      where wf is the forcing frequency
c
c**************************************************************************
c**************************************************************************
c**************************************************************************
      ...
      CHARACTER(LEN=256) :: fmodel, frename, funname
      INTEGER :: nmodes
      INTEGER :: ll0(2), nn0(2), jcom0
...
c      eps0=max(eps0,1.d-12)
      ...
      open(7,file=fmodel,status='old',form='formatted',ACTION='READ')!,iostat=ierr)
      open(8,file=frename,form='formatted',ACTION='write')!,iostat=ierr)
      open(9,file=funname,form='unformatted',ACCESS='SEQUENTIAL',
     + ACTION='write')
      WRITE(9) 'VFUN1.02'
      WRITE(9) jcom0,REAL(ww0(1)),REAL(ww0(2)),ll0(1),ll0(2),
     + REAL(wgrav0)
      call model(7,8,9)
      close(7)
      ...
      subroutine wtable(iout,ioeig,ifreq,ncall,jcom0,ll0,ww0,nn0,eps0,
     + wgrav0)
c*** makes up table of frequencies ***
      implicit real*8(a-h,o-z)

      INTEGER :: jcom0, ll0(2), nn0(2)
...
c*** find roots by spline interpolation ***
     ...
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
      common/mtab/we(2),de(2),ke(2),wtry,bm
      dimension x(20),det(20),qx(3,20),wrk(3,20),wt(*),kchar(4)
      character*2 kchar
...
c      cvel=b*rn/(l+0.5)/1000.
      ...
      if(ifreq.eq.1) then
        ...
        gcom=vn*cg/1000.d0
        qmod=0.d0
c HL2 ***** update the q of a mode to the adjusted value:
c           original: qmod=1.d0/qinv
c           this needs to account for the forcing frequency
        if(qinv.gt.0.d0) qmod=(1.d0/qinv)*(wf/1.9415d-3)**alpha
...
c        common/eifx/a(14,mk),ndum(mk) !HL,jjj(mk),vsh(mk),eta(mk),wrk(mk*10)
        common/eifx/a(14,mk),ndum(mk)
        common/eigsav/krec,nrec,nrecic,nrecoc
        common/counter/icount,ibad
...
c           Using trapzoidal method to get integrand
          ...
          do i=1,n
            if(krec(i).eq.1) then
              ...
              abuf(j+4*nr+jst)=factor*(sumall-SO(j))
              abuf(j+5*nr+jst)=-factor*pU(j)
            endif
          end do
        elseif(jcom.eq.2) then
          do i=noc+1,n
...
c
     ...
     +  eps,fl,
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
c HL      common/eifx/vpv(mk),vph(mk),vsv(mk),vsh(mk),eta(mk),wrk(mk*10)
      real*8 vpv(mk),vph(mk),vsv(mk),vsh(mk),eta(mk),wrk(mk*10)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      common/eigsav/krec,nrec,nrecic,nrecoc
...
c                                   write model into binary file
        ...
        write(ioeig) nrec,nrecic,nrecoc,ifanis,real(tref),(rrec(i),
     +    i=1,nrec),
...
c*** normalise and spline ***
     ...
     +    -1.5d0*rat)
      ...
      OPEN(UNIT=20,FILE='gspl.out')
      WRITE(UNIT=20,FMT='(3E15.7)') (qro(1:3,i),i=1,n)
      CLOSE(UNIT=20)
      call grav(g,rho,qro,r,n)
      OPEN(UNIT=21,FILE='grav.out')
      WRITE(UNIT=21,FMT='(E15.7)') (g(i),i=1,n)
      CLOSE(UNIT=21)
...
c      WRITE(ioeig) g(n)*gn
      WRITE(*,'(6X,A,2(E23.16,A))') 'gravity at surface =', 
     c  g(n)*gn ,' m/s^2',
     c  g(n),' non-dimensional'
...
c**** of the secular determinant as det and the count of zero crossings.
     ...
     +  eps,fl,
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
c      common/eifx/a(14,mk),dum(mk) !HL,jjj(mk),vsh(mk),eta(mk),wrk(mk*10)
      common/eifx/a(14,mk),ndum(mk)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
...
c HL1 ********************************************************
c ************************************************************      
c **  H. LAU 03/2018: edit fct to take tide frequency into  **
c **  account in factor.                                    **
c **  here, tref has been converted from period of cycle    **
c **  to period of angular cycle (i.e. tref=0.5*tref/pi)    **
c **  and so we need to multiply by angular frequency of    **
c **  tide.                                                 **
c **  if(tref.gt.0.d0) fct=2.d0*dlog(tref*wdim)/pi          **
c **  The goal is to map the elastic parameters from tref   **
c **  to that of the forcing frequency.                     **
c **  This form must match what is in the main code         **
c **  NOTE - the imaginary part of the eigenfrequency is    **
c **  evaluated with a constant frequency Q model and must  **
c **  be adjusted outside this code                         **

      wm=1.9415d-3
      wf=2.d0*pi/(12.d0*60.d0*60.d0) 
      alpha=0.3d0
      tref1=(tref*pi/0.5d0)  ! now tref is just the period
      wref=2.d0*pi/tref1     ! angular frequency
      alpex=0.9999072901761793d0 ! = (1.-wm/wf)**alpha 
                                 ! for some reason fortran cries

      if(tref.gt.0.d0) fct = 2.d0*dlog(wm/wref)+alpex/alpha
c ************************************************************
c HL1 ********************************************************
c ************************************************************
...
c*** propagate a minor vector in a solid region from level jf to jl ***
     ...
     +  eps,fl,
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
c      common/eifx/ar(14,mk),inorm(mk) !HL ,jjj(mk),vsh(mk),eta(mk),wrk(mk*10)
      common/eifx/ar(14,mk),inorm(mk)
      common/shanks/b(46),c(10),dx,step(8),stepf,maxo,in
...
c*** massages spheroidal mode eigenfunctions before output ***
     ...
     +  eps,fl,
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
c      common/eifx/a(14,mk),inorm(mk) !HL /,jjj(mk),vsh(mk),eta(mk),wrk(mk*10)
      common/eifx/a(14,mk),inorm(mk)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      dimension zi(4)
...
c*** given rho and spline coeffs,computes gravity ***
      ...
      parameter (mk=30000)
      save
c      dimension g(1),rho(1),qro(3,1),r(1)  !HL 
      dimension g(mk), rho(mk),qro(3,mk),r(mk)
      g(1)=0.d0
      do i=2,n
        im1=i-1
        del=r(i)-r(im1)
...
c   fini.
 13   return
      ...
      implicit real*8(a-h,o-z)
      save
c      dimension x(1),y(1),q(3,1),f(3,1),yy(3)
      dimension x(20), y(20),q(3,20),f(3,20),yy(3)
      equivalence (yy(1),y0)
      data yy/3*0.d0/
      a0=0.d0
      j2=n-2
...
c*** interpolates integrands for normalisation,cg,q etc..for use with gauslv.
     ...
     +  eps,fl,
     +  fl1,fl2,fl3,sfl3,jcom,nord,l,kg,kount,knsw,ifanis,iback
c      common/eifx/ar(14,mk),dum(mk) !! HL ,jjj(mk),vsh(mk),eta(mk),wrk(mk*10)
      common/eifx/ar(14,mk),ndum(mk)
